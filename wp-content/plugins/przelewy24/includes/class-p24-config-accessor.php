...
class P24_Config_Accessor {
	...
	private function fix_booleans() {
		$to_fix       = array(
			...
			'sub_enabled',
			'p24_admin_mail_early_on_new_order',
			'p24_client_mail_on_hold_at_pending',
		);
		$default_true = array(
			'pass_cart',
		);
		foreach ( $to_fix as $one ) {
			if ( null === $this->config->{$one} ) {
				$this->config->{$one} = in_array( $one, $default_true, true );
	 ...
	 */
	public function get_pass_cart() {
		try {
			$pass_cart = $this->config->pass_cart;
			return $this->to_selected_bool( $pass_cart );
		} catch ( InvalidArgumentException $ex ) {
			$msg = 'The internal representation of conceal_cart is invalid.';
			throw new LogicException( esc_html( $msg ) );
		}
	}

	/**
	 * Set pass_cart in config.
	 *
	 * The input is expected to be in selected representation.
	 *
	 * @param bool|string $pass_cart The new pass_cart.
	 * @return P24_Config_Accessor
	 * @throws InvalidArgumentException If input is in wrong representation.
	 */
	public function set_pass_cart( $pass_cart ) {
		$this->config->pass_cart = $this->from_selected_bool( $pass_cart );
		return $this;
	}
	...
	/**
	 * Get p24_admin_mail_early_on_new_order from config
	 *
	 * @return bool|string
	 */
	public function get_admin_mail_early_on_new_order() {
		$admin_mail_early_on_new_order = $this->config->p24_admin_mail_early_on_new_order;
		return $this->to_selected_bool( $admin_mail_early_on_new_order );
	}
	 ...
	 * @return bool|string
	 */
	public function get_client_mail_on_hold_at_pending() {
		$client_mail_on_hold_at_pending = $this->config->p24_client_mail_on_hold_at_pending;
		return $this->to_selected_bool( $client_mail_on_hold_at_pending );
	}
}
