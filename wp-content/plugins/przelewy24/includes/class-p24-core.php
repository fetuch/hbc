...
class P24_Core {
	...
	/**
	 * Class to enforce some mails.
	 *
	 * @var P24_Email_Enforcer
	 */
	private $email_enforcer;
	...
	public function __construct() {
		...
		$this->update           = new P24_Update();
		$this->email_enforcer   = new P24_Email_Enforcer( $this );
	 ...
	 *
	 * @param mixed $probably_null Default value from filter.
	 * @return P24_Core
	 */
	public function get_this_if_null( $probably_null ) {
		return $probably_null ? $probably_null : $this;
	}
	...
	public function after_main_gateway_initiation() {
		...
		$this->card_html      = new P24_Card_Html( $this );
	}
	 ...
	 *
	 * @param bool     $by_default Default value.
	 * @param WC_Order $order The order to check.
	 *
	 * @return mixed
	 */
	public function check_if_cancellable( $by_default, $order ) {
		if ( $by_default ) {
			return $by_default;
	 ...
	 *
	 * @param string $s Stringified float.
	 *
	 * @return int
	 */
	public static function convert_stringified_float_to_cents( $s ) {
		list( $fulls, $cents ) = explode( '.', $s );
	...
	/**
	 * Extract locale from global scope.
	 *
	 * @return string
	 */
	public function extract_locale() {
		$locale = null;
		if ( isset( $GLOBALS['locale'] ) ) {
			$locale = $GLOBALS['locale'];
			if ( preg_match( '/^([a-zA-Z]+)/', $locale, $m ) ) {
	...
	/**
	 * Get class to enforce emails.
	 *
	 * @return P24_Email_Enforcer
	 */
	public function get_email_enforcer() {
		return $this->email_enforcer;
	}
}
