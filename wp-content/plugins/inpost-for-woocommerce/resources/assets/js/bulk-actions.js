...
jQuery(document).ready(function(){
    ...
    jQuery('#doaction').on('click',function(e) {
        ...
        if(action === 'easypack_bulk_create_shipments_then_labels') {
            e.preventDefault();
            let selected_data = animate_table_processing();
            if( typeof selected_data != 'undefined' && selected_data !== null ) {
                if( Object.keys(selected_data.orders).length ) {
                    process_selected_item( selected_data.orders, 1, selected_data.selected_row_count, form, 0, 1 );
                }
            }
        }
        ...
        if( action === 'easypack_bulk_create_shipments' ) {
            ...
            if( typeof selected_data != 'undefined' && selected_data !== null ) {
                if( Object.keys(selected_data.orders).length ) {
                    process_selected_item( selected_data.orders, 1, selected_data.selected_row_count, form, 0, 0 );
                }
            }
        }
...
function process_selected_item(orders, index, total, form, failed, need_labels = false) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (data.status === 'ok') {
                ...
                if (index <= total) {
                    process_selected_item(orders, (index + 1), total, form, failed, need_labels);
                }
...
function animate_table_processing() {
    var form = jQuery('#posts-filter'),
